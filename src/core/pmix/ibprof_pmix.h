...
#define POST_PROF(func_name) \
	ibprof_update(IBPROF_MODULE_PMIX, TBL_CALL_NUMBER(func_name), \
...
#define POST_ERR(func_name) \
	ibprof_update_ex(IBPROF_MODULE_PMIX, TBL_CALL_NUMBER(func_name), \
 ...
 */
#define PRE_(func_name) f = pmix_module_context.mean.func_name;
...
#define FUNC_BODY_INT(type, func_name, ...)     \
    ...
    f = pmix_module_context.noble.func_name;                            \
    PRE_##type(func_name)                                               \
    INTERNAL_CHECK();                                                   \
    ret = f(__VA_ARGS__);                                               \
...
#define TBL_CALL_NUMBER(func_name) \
        offsetof(struct pmix_module_api_t, func_name) / sizeof(void*)
